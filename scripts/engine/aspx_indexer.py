...
def build_index(repo_data: dict, repo_name: str, repo_path: str) -> dict:
    ...
    nav_map          = _build_navigation_map(pages)
...
def _link_usages(pages: List[dict], controls: List[dict], masters: List[dict]) -> None:
    ...
    # Build lookup maps: normalised rel_path/filename → index in list
    ctrl_map: Dict[str, int] = {}
    for i, c in enumerate(controls):
        ctrl_map[_normalise_path(c['rel_path'])] = i
        ctrl_map[c['filename'].lower()] = i

    master_map: Dict[str, int] = {}
    for i, m in enumerate(masters):
        master_map[_normalise_path(m['rel_path'])] = i
        master_map[m['filename'].lower()] = i
    ...
    for page in pages:
        ...
        if mp:
            idx = master_map.get(mp) or master_map.get(Path(mp).name.lower())
...
def _build_navigation_map(pages: List[dict]) -> Dict[str, List[str]]:
    """Build page→page navigation adjacency (based on HyperLinks and Response.Redirect)."""
    ...
    for page in pages:
        ...
        for link in links:
            ...
            # Keep only internal .aspx references
